...
class OrphanedWorkItemCleanupServiceSpec
  ...
  private val repository =
    new SubscriptionWorkItemRepository(repoConfig, mongoComponent)

  private val appConfig = mock[AppConfig]

  private val auditService =
    new LegacySubscriptionAuditService(mockAuditService)
  ...
  "cleanup" should {
    ...
    "mark eligible work items permanently failed" in {
      ...
      val persisted =
        repository.coll.find().first().toFuture().futureValue
