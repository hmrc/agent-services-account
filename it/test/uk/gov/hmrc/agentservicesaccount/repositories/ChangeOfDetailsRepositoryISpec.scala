  ...
  "find" should :
    "return Some(changeOfDetailsRequest)" when :
      "a matching record is available in the DB" in :
        repository.collection.insertOne(testChangeOfDetailsRequest).head().futureValue

        val result = repository.find("AARN1234567").futureValue
  ...
  "upsert" should :
    "return UpdateResult" when :
      "the record is successfully inserted in the DB" in :
        val result = repository.upsert(testChangeOfDetailsRequest).futureValue
      ...
      "the record is successfully updated in the DB" in :
        ...
        val result = repository.upsert(updatedChangeOfDetailsRequest).futureValue
        result.wasAcknowledged() shouldBe true

        val updatedResult = repository.find("AARN1234567").futureValue
  ...
  "delete" should :
    "return DeleteResult with a count of 1" when :
      "a matching record is available in the DB" in :
        ...
        val result = repository.delete("AARN1234567").futureValue
